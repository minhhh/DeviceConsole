...
{
    public class DeviceConsole : SceneSingleton<DeviceConsole >
    {
        ...
        [SerializeField] private bool autoFocusInputField = true;
        ...
        private void Awake ()
        {
            ...
            DebugCommands.Instance.Commands.AddRange (FindMethodAttributes (
                new Assembly[] { 
                    Assembly.GetExecutingAssembly () 
                }));
        }

        public static DebugCommands.Command[] FindMethodAttributes (Assembly[] assemblies)
        {
            List<DebugCommands.Command> result = new List<DebugCommands.Command> ();
            ...
            for (int current = 0; current < assemblies.Length; current++) {
                MethodInfo[] methods = assemblies [current].GetTypes ()
                    ...
                    .Where (m => m.GetCustomAttributes (typeof(DebugCommandAttribute), true).Length > 0)
                    .ToArray ();
                ...
                for (int i = 0; i < methods.Length; i++) {
                    ...
                    DebugCommandAttribute attribute = ExtractAttribute<DebugCommandAttribute> (methods [i]);
                    var cmd = new DebugCommands.Command ();

                    cmd.name = attribute.Name;
                    cmd.method = methods [i];
                    cmd.description = attribute.Description;
                    cmd.argsDescription = attribute.ArgsDescription;
        ...
        [DebugCommandAttribute ("plogs", "Clears the console and prints the logs in the specified range", "<start_index> <end_index>")]
        private static void PLogs (string startIndex, string endIndex)
        {
            ...
            if (!int.TryParse (endIndex, out end)) {
            ...
            for (int i = start; i <= end; i++) {
                Instance.PrintToConsole (DebugLogs.Instance.Logs [i], i + ":");
            }
        }

        [DebugCommandAttribute ("setAF", "Sets the boolean value of the Auto Focus Input Field property.", "< true | false >")]
        private static void SetAF (string autoFocus)
        {
            if (autoFocus == "t" || autoFocus == "true") {
                DeviceConsole.Instance.autoFocusInputField = true;
            } else if (autoFocus == "f" || autoFocus == "false") {
                DeviceConsole.Instance.autoFocusInputField = false;
            } else {
                Debug.LogError ("Please specify either t or f.");
            }
        }

        [DebugCommandAttribute ("help", "Prints list of commands")]
        ...
        [DebugCommandAttribute ("clear", "Clears all text from the debug console")]
        private static void Clear ()
        {
            DebugLogs.Instance.ClearLogs ();
        }

        [DebugCommandAttribute ("history", "Prints list of all previous commands")]
        ...
        [DebugCommandAttribute ("logc", "Prints the number of logs currently held by DebugLogs")]
        private static void PrintLogCount ()
        {
            Debug.Log (string.Format ("There are {0} reported logs.", DebugLogs.Instance.Logs.Count));
        }

        [DebugCommandAttribute ("loadscene", "Loads the scene with the given name", "<scene_name>")]
        private static void LoadScene (string sceneName)
        {
            Application.LoadLevel (sceneName);
        }

        [DebugCommandAttribute ("quit", "Closes the app")]
